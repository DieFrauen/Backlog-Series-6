...
function c26067007.initial_effect(c)
	--pendulum summon
	...
	e1:SetTarget(c26067007.detg)
	e1:SetOperation(c26067007.deop)
	c:RegisterEffect(e1)
end
function c26067007.detg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFieldGroupCount(tp,LOCATION_PZONE,LOCATION_PZONE)>0 end
	local g=Duel.GetFieldGroup(tp,LOCATION_PZONE,LOCATION_PZONE)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,#g,0,0)
end
function c26067007.pcfilter(c)
...
function c26067007.rescon(sg,e,tp,mg)
...
function c26067007.deop(e,tp,eg,ep,ev,re,r,rp)
	...
	local hg=Duel.GetMatchingGroup(c26067007.thfilter,tp,LOCATION_DECK,0,nil)
	local p1=Duel.CheckPendulumZones(tp)
	and aux.SelectUnselectGroup(pg,e,tp,2,2,c26067007.rescon,0)
	...
	if (p1 or p2) and Duel.SelectYesNo(tp,aux.Stringid(26067007,0)) then
		...
		if op==1 then
			...
			local sg=aux.SelectUnselectGroup(pg,e,tp,2,2,c26067007.rescon,1,tp,HINTMSG_TOFIELD)
