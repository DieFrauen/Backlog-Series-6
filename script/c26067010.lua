...
function c26067010.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(26067010,0))
	e2:SetCategory(CATEGORY_TOGRAVE+CATEGORY_DESTROY)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetProperty(EFFECT_FLAG_NO_TURN_RESET)
	e2:SetRange(LOCATION_SZONE)
	e2:SetCountLimit(1,0,EFFECT_COUNT_CODE_SINGLE)
	e2:SetTarget(c26067010.tgtg)
	e2:SetOperation(c26067010.tgop)
	c:RegisterEffect(e2)
...
function c26067010.pcfilter(c)
	return c:IsSetCard(0x667) and c:IsType(TYPE_PENDULUM) and c:IsFaceup() and c:IsAbleToGrave() and c:IsAbleToChangeControler()
end
function c26067010.tgtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) end
	if chk==0 then return Duel.IsExistingTarget(c26067010.pcfilter,tp,LOCATION_MZONE,0,1,nil) end
...
function c26067010.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsControler(tp) then
		...
		e1:SetDescription(aux.Stringid(26067010,3))
		e1:SetLabelObject(tc)
		e1:SetOperation(c26067010.redir)
		e1:SetReset(RESET_EVENT+RESETS_STANDARD)
		c:RegisterEffect(e1)
	end
end
function c26067010.redir(e,tp,eg,ep,ev,re,r,rp)
