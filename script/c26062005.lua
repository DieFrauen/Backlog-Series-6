--Blazon - Sablemander the Standard
function c26062005.initial_effect(c)
	c:SetSPSummonOnce(26062005)
	--special summon
...
function c26062005.dcost(c,tp)
	return (c:IsSetCard(0x662) or (c:IsAttribute(ATTRIBUTE_FIRE)
	and c:IsMonster()))
	and Duel.IsPlayerCanRelease(tp,c)
end
function c26062005.otcost(c,tc)
	return c:IsCode(tc:GetCode()) and c:IsFaceup()
end
function c26062005.dkcost(c,tp)
	return c:IsSetCard(0x662) and c:IsMonster() and not c:IsType(TYPE_TUNER) and Duel.IsPlayerCanRelease(tp,c) and not Duel.IsExistingMatchingCard(c26062005.otcost,tp,LOCATION_GRAVE+LOCATION_ONFIELD,0,1,nil,c) and not c:IsCode(26062005)
end
function c26062005.spcon(e,c)
	local sc=e:GetHandler()
	if c==nil then return true end
	local tp=c:GetControler()
	local g1=Duel.GetMatchingGroup(c26062005.dcost,tp,LOCATION_HAND+LOCATION_ONFIELD,0,c,tp)
	local g2=Duel.GetMatchingGroup(c26062005.cfcost,tp,LOCATION_GRAVE,0,nil)
	local g3=Duel.GetMatchingGroup(c26062005.dkcost,tp,LOCATION_DECK,0,c,tp)
	g1:Merge(g2)
...
function c26062005.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local sc=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local g1=Duel.GetMatchingGroup(c26062005.dcost,tp,LOCATION_HAND+LOCATION_ONFIELD,0,sc,tp)
	local g2=Duel.GetMatchingGroup(c26062005.cfcost,tp,LOCATION_GRAVE,0,nil)
	local g3=Duel.GetMatchingGroup(c26062005.dkcost,tp,LOCATION_DECK,0,sc,tp,e:GetHandler())
	if #g3>0 and Duel.IsPlayerAffectedByEffect(tp,26062007) and Duel.GetFlagEffect(tp,26062007)==0
	then g1:Merge(g3) end
	local sc=nil
