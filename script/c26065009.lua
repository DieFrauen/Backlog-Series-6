...
function c26065009.initial_effect(c)
	...
	e2:SetCountLimit(1,{26065009,1},EFFECT_COUNT_CODE_OATH)
	...
	e3:SetCountLimit(1,{26065009,2},EFFECT_COUNT_CODE_OATH)
...
function c26065009.target2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	local rg=Duel.GetMatchingGroup(c26065009.filter2,tp,LOCATION_GRAVE+LOCATION_HAND+LOCATION_ONFIELD,LOCATION_GRAVE,c)
	if chkc then return chkc:IsOnField() and chkc~=c end
	if chk==0 then
		if e:GetLabel()==1 then
			e:SetLabel(0)
