...
function c26067207.initial_effect(c)
	--pendulum summon
	...
	e1:SetTarget(c26067207.detg)
	e1:SetOperation(c26067207.deop)
	c:RegisterEffect(e1)
end

function c26067207.detg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetFieldGroupCount(tp,LOCATION_PZONE,LOCATION_PZONE)>0 end
	local g=Duel.GetFieldGroup(tp,LOCATION_PZONE,LOCATION_PZONE)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,#g,0,0)
end
function c26067207.pcfilter(c)
...
function c26067207.deop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetFieldGroup(tp,LOCATION_PZONE,LOCATION_PZONE)
	local ct=Duel.Destroy(g,REASON_EFFECT)
	local pg=Duel.GetMatchingGroup(c26067207.pcfilter,tp,LOCATION_EXTRA,0,nil)
	local mg=Duel.GetMatchingGroup(c26067207.mfilter,tp,LOCATION_EXTRA,0,nil,e)
	local cond1=Duel.CheckPendulumZones(tp) and #pg>=2
	local cond2=Duel.IsExistingMatchingCard(c26067207.fusion,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg)
	if #pg>0 and (cond1 or cond2) and Duel.SelectYesNo(tp,aux.Stringid(26067207,0)) then
		local op=Duel.SelectEffect(tp,
		{cond1,aux.Stringid(26067207,1)},
		{cond2,aux.Stringid(26067207,2)})
		if op==1 then
		...
		elseif op==2 then
			local fc=Duel.SelectMatchingCard(tp,c26067207.fusion,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,mg):GetFirst()
