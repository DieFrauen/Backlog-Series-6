...
function c26067207.deop(e,tp,eg,ep,ev,re,r,rp)
	...
	local hg=Duel.GetMatchingGroup(c26067207.thfilter,tp,LOCATION_DECK,0,nil)
	local p1=Duel.CheckPendulumZones(tp)
	and aux.SelectUnselectGroup(pg,e,tp,2,2,c26067207.rescon,0)
	...
	if (p1 or p2) and Duel.SelectYesNo(tp,aux.Stringid(26067007,0)) then
		...
		if op==1 then
			...
			local sg=aux.SelectUnselectGroup(pg,e,tp,2,2,c26067207.rescon,1,tp,HINTMSG_TOFIELD)
