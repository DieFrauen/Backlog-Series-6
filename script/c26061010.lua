...
function c26061010.initial_effect(c)
	...
	e1:SetCost(c26061010.cost)
	e1:SetTarget(c26061010.target)
	e1:SetOperation(c26061010.operation)
	c:RegisterEffect(e1)
	--act in hand
	...
	e3:SetCountLimit(1,26061010,EFFECT_COUNT_CODE_OATH)
	e3:SetHintTiming(TIMING_END_PHASE,TIMING_END_PHASE)
	e3:SetCost(aux.bfgcost)
	e3:SetCondition(c26061010.lpcond)
	e3:SetOperation(c26061010.lpop)
	c:RegisterEffect(e3)
...
function c26061010.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c26061010.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c26061010.filter1(chkc,e,tp,tid) end
	if chk==0 then
		return Duel.IsExistingMatchingCard(c26061010.filter1,tp,LOCATION_ONFIELD,0,1,nil) or Duel.IsExistingMatchingCard(c26061010.filter2,tp,LOCATION_GRAVE,0,1,nil,e,tp,tid)
	end
	local g1=Duel.GetMatchingGroup(c26061010.filter1,tp,LOCATION_ONFIELD,0,nil)
	local g2=Duel.GetMatchingGroup(c26061010.filter2,tp,LOCATION_GRAVE,0,nil,e,tp,tid)
	g1:Merge(g2)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g1,1,0,0)
end
function c26061010.operation(e,tp,eg,ep,ev,re,r,rp)
	local tid=Duel.GetTurnCount()
	local g1=Duel.GetMatchingGroup(c26061010.filter1,tp,LOCATION_ONFIELD,0,nil)
	local g2=Duel.GetMatchingGroup(c26061010.filter2,tp,LOCATION_GRAVE,0,nil,e,tp,tid)
	g1:Merge(g2)
...
function c26061010.lpcond(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetLP(tp)~=c26061010[tp] and c26061010[tp]>0
end
function c26061010.lpop(e,tp,eg,ep,ev,re,r,rp)
	Duel.SetLP(tp,c26061010[tp])
end