...
function c26063005.initial_effect(c)
	...
	e7:SetCondition(aux.IsGeminiState)
...
function c26063005.gemini(e)
	...
	return not c:IsGeminiState() and (c:GetLocation()==LOCATION_HAND or c:IsFaceup())
end
function c26063005.infilter(c)
	Card.IsHasEffect(c,26063005)
end
function c26063005.gfilter(c)
	return c:IsSummonable(true,nil) and c:IsType(TYPE_NORMAL) and not c:IsGeminiState()
end
