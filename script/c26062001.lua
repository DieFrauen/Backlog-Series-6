...
function c26062001.dcost(c,tp)
	return (c:IsSetCard(0x662) or (c:IsAttribute(ATTRIBUTE_FIRE)
	and c:IsMonster()))
	and Duel.IsPlayerCanRelease(tp,c)
end
function c26062001.otcost(c,tc)
	return c:IsCode(tc:GetCode()) and c:IsFaceup()
end
function c26062001.dkcost(c,tp)
	return c:IsSetCard(0x662) and c:IsMonster() and not c:IsType(TYPE_TUNER) and Duel.IsPlayerCanRelease(tp,c) and not Duel.IsExistingMatchingCard(c26062001.otcost,tp,LOCATION_GRAVE+LOCATION_ONFIELD,0,1,nil,c) and not c:IsCode(26062001)
end
function c26062001.spcon(e,c)
	local sc=e:GetHandler()
	if c==nil then return true end
	local tp=c:GetControler()
	local g1=Duel.GetMatchingGroup(c26062001.dcost,tp,LOCATION_HAND+LOCATION_ONFIELD,0,c,tp)
	local g2=Duel.GetMatchingGroup(c26062001.cfcost,tp,LOCATION_GRAVE,0,nil)
	local g3=Duel.GetMatchingGroup(c26062001.dkcost,tp,LOCATION_DECK,0,c,tp)
	g1:Merge(g2)
...
function c26062001.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g1=Duel.GetMatchingGroup(c26062001.dcost,tp,LOCATION_HAND+LOCATION_ONFIELD,0,sc,tp)
	local g2=Duel.GetMatchingGroup(c26062001.cfcost,tp,LOCATION_GRAVE,0,nil)
	local g3=Duel.GetMatchingGroup(c26062001.dkcost,tp,LOCATION_DECK,0,sc,tp,e:GetHandler())
	if #g3>0 and Duel.IsPlayerAffectedByEffect(tp,26062007) and Duel.GetFlagEffect(tp,26062007)==0
	then g1:Merge(g3) end
	local sc=nil
