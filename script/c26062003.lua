...
function c26062003.spcon(e,c)
	...
	local b1=Duel.IsExistingMatchingCard(c26062003.cfilter,tp,LOCATION_HAND,0,1,c)
	local b2=Duel.IsExistingMatchingCard(c26062003.cfcost,tp,LOCATION_GRAVE,0,1,nil)
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and (b1 or b2)
end
function c26062003.spop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
	local b1=Duel.GetMatchingGroup(c26062003.cfilter,tp,LOCATION_HAND,0,c)
	local b2=Duel.GetMatchingGroup(c26062003.cfcost,tp,LOCATION_GRAVE,0,nil)
	if b2:GetCount()>0 and (b1:GetCount()==0 or Duel.SelectYesNo(tp,aux.Stringid(26062011,3))) then
		local tg=Duel.GetFirstMatchingCard(c26062003.cfcost,tp,LOCATION_GRAVE,0,nil)
...
function c26062003.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	e1:SetLabel(ch)
	e1:SetCost(c26062003.costchk)
	e1:SetOperation(c26062003.costop)
...
function c26062003.grop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) then
		if tc:IsLevelBelow(1) and Duel.SelectYesNo(tp,aux.Stringid(26062003,1)) then
			local e1=Effect.CreateEffect(c)
...
function c26062003.costchk(e,te,tp)
	local ch=e:GetLabel()
	local op=te:GetHandlerPlayer()
	return Duel.GetCurrentChain()==ch or (Duel.IsPlayerAffectedByEffect(op,26062010))
end
function c26062003.costop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,26062003)
	Duel.Damage(tp,300,REASON_COST)
end